classdef lineMapLocalizer < handle
    ...
    methods
        ...
        function [curr_error, J] = getJacobian(obj,poseIn,modelPts)
            ...
            eps = 1e-9;
            dx = [eps ; 0.0 ; 0.0];
            dx_error = obj.fitError(pose(poseIn.getPose + dx), modelPts);
            dEdx = (dx_error - curr_error)/eps;
            
            dy = [0.0; eps; 0.0];
            dy_error = obj.fitError(pose(poseIn.getPose + dy), modelPts);
            dEdy = (dy_error - curr_error)/eps;
            
            dTh = [0.0; 0.0; eps];
            dth_error = obj.fitError(pose(poseIn.getPose + dTh), modelPts);
