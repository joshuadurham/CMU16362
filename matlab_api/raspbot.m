...
%        fileWrite_pub;
    ...
    methods
        ...
        %RASPBOT Initializes a RaspBot in simulation or connects to and sets up ROS
        % topics for a real robot.
        %
        %   robot = RASPBOT() initializes a robot at the origin.
        %
        %   robot = RASPBOT(name, pose) initializes a robot named 'name' at
        %   a pose given in format [x; y; theta].
        %
        %   robot = RASPBOT('sim', pose) initializes a simulated robot at the
        %   given pose.
            if(nargin == 0)
                r.name = 'RaspBot';
            elseif(nargin > 0)
...
%                r.fileWrite_pub=rospublisher('/fileWrite', 'audio_receiver/audioFile');
        ...
        function shutdown(r)
        %SHUTDOWN Stops simulation or shuts down ROS.           
            if r.sim
                stop(r.update_timer);
		...
		function forksUp(r)
        %FORKSUP Moves forklifts to raised position.
			if r.sim
                fprintf('forks up!\n');
        ...
        %FORKSDOWN Moves forklifts to lowered position.            
			if r.sim
				fprintf('forks down!\n');
                %warning('No forks in simulation... yet');
			else
        ...
        %MANUAL_UPDATE Updates the simulation.          
			toc(r.last_update)
        ...
        function say(r, str)
        %SAY Commands the robot to say a phrase from its speakers.  The
        % phrase must be a string, but the string can include numerals
        % (str = 'I have 3 apples' is a valid input, but str = 3 is not).
        %
...
%          function play(r, str)
%          %PLAY Commands the robot to play a sound file from its speakers.
%          %
%          %    robot.PLAY(str) causes the robot to play the file saved
%          %    under the name str.  The file must first be saved to the robot
%          %    using the sendSoundFile function.
%              if (r.sim)
%                  [y, f] = audioread(str);
%                  sound(y,f);
%              else
...
%          %SENDSOUNDFILE Sends a local sound file to the robot and saves
%          % it under the specified name.  This function only works with 
%          % .wav files.
%          %
%          %    robot.SENDSOUNDFILE(localPath, remotePath) saves a sound
%          %    file located at localPath to the robot with the path remotePath.
%          %    To play this file from the robot, use robot.play(remotePath).
%             if (r.sim)
%                 return
...
%          end
         ...
         %IMAGE Captures an image from the forward-facing camera on the robot.
        ...
        %GENMAP Adds a map for use in laser simulation.       
        ...
        %TOGGLEPLOT Changes which display list is being shown.               
            if ishandle(r.ph(1))
	...
	methods (Hidden = true, Access = 'private')
        ...
        function simUpdate(r,caller,event)
			...
			%Every fifth update, update the encoders
