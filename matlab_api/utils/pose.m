classdef pose < handle
    % A layer over a column vector that provides access methods and
    ...
    methods(Static = true)
        ...
        %MATTOPOSEVEC Convert a homogeneous transform into a vector that can be
    ...
    methods(Access = public)
        ...
        %POSE Sets the pose of an object.
        %
        %   pose = POSE() sets the pose of an object to be the origin.
        %
        %   pose = POSE(x, y, theta) sets the location of the object to be the
        %   coordinate (x,y) with the orientation theta.
            if(nargin == 1)
                sz = size(x);
                if(sz(1) == 1 || sz(2) == 1)
                   obj.poseVec = x;
        ...
        %BTOA Returns the homogeneous transform matrix that converts coordinates from
        ...
        %ATOB Returns the homogeneous transform matrix that converts coordinates from
        ...
        %BTOAROT Returns the rotation matrix that converts coordinates from
        % the b frame to the a frame.
       ...
       function mat = aToBRot(obj)
        %ATOBROT Returns the rotation matrix that converts coordinates from
        % the a frame to the b frame.
